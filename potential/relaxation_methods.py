...
def sor(grid, threshold):
    ...
    while np.amax(new_grid - old_grid) > threshold or iter == 0:
        ...
        # if iter % 5 == 0:
        checkpoints.append(new_grid.copy())

    return new_grid, checkpoints, iter
...
grid = np.zeros((nx, ny))
sor_final, sor_checkpoints, iter_sor = sor(grid, eps)
...
for i, checkpoint in enumerate(sor_checkpoints):
    plot3D(checkpoint, "Task1: successive overrelaxation after 100 iterations", f"results/sor_{i}")
plot3D(sor_final, "Task1: successive overrelaxation final result", "results/sor_final")
